...
// Unconditional import so Vite/Rollup never tree-shakes the polyfill away,
// even though it is only invoked at runtime when crypto.subtle is absent.
import { hmacSha256Hex as hmacSha256HexPolyfill } from "./sha256-polyfill.js";
...
async function hmacSha256Hex(input: string): Promise<string> {
  ...
  // Fallback: pure-JS HMAC-SHA256 for browsers without Web Crypto
  // (e.g., Tesla in-car browser, some embedded/kiosk browsers).
  return hmacSha256HexPolyfill(input, APP_SIGNING_KEY);
}
